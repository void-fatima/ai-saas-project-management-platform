...
Completion is displayed as DONE roots / total roots and DONE subtasks / total subtasks. A completed subtask does not mark its parent done; subtasks do not inflate root completion. Zero work is an empty state, not a fabricated productivity score. Archived projects count in project totals but their tasks do not contribute to current workload. Unread counts remain in the existing own-user notification bell. Due dates, overdue work, trends, project health, workload analysis and productivity claims are unsupported by this schema and remain deferred.
